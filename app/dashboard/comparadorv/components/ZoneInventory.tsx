  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDragging
          ? 'bg-blue-600/20 border-blue-500 ring-2 ring-blue-500/20 shadow-2xl scale-105'
          : 'bg-zinc-900/50 border-zinc-800/50 hover:border-zinc-600 hover:bg-zinc-900'}
      `}
    >
...
export const ZoneInventory = () => {
  ...
  if (!datasetId) return (
    <div className="flex flex-col items-center justify-center py-20 px-6 text-center border border-dashed border-zinc-800 rounded-2xl m-4">
  ...
  return (
    <div className="flex flex-col h-full max-h-175 bg-zinc-950/20 rounded-2xl border border-zinc-800/50 overflow-hidden">
      <div className="p-4 border-b border-zinc-800 bg-zinc-900/30 backdrop-blur-md sticky top-0 z-10">
      ...
      <div className="flex flex-col gap-2 p-4 overflow-y-auto custom-scrollbar">
        {inventoryZones.length === 0 ? (
           <div className="space-y-3">
             {[...Array(6)].map((_, i) => (
               <div key={i} className="h-15 bg-zinc-900/50 rounded-xl animate-pulse border border-zinc-800/50" />
             ))}
           </div>
        ) : (
